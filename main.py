...
async def clone_server():
    ...
    print("\n> Sunucu Kopyalama Başarıyla Tamamlandı! " +
...
class ClonerBot:
  ...
  def edit_settings_function(self):
    ...
    categories = Confirm.ask("> Kategoriler?")
    channels = Confirm.ask("> Kanallar?")
    roles = Confirm.ask("> Roller?")
    emojis = Confirm.ask("> Emojiler?")
  ...
  def main(self):
    ...
    if self.data["token"] == False:
      ...
      print("> Token Bulundu")
    self.clear()
    edit_settings = Confirm.ask("\n> Ayarları değiştirmek ister misiniz?")
    self.clear()
    if edit_settings:
      self.edit_settings_function()
    self.clear()

    self.GUILD = Prompt.ask('\n> Düzenlemek istediğiniz Sunucunun ID sini girin (Sunucuyu discord üzerinden kendiniz oluşturunuz!)')
    sleep(0.5)

    self.INPUT_GUILD_ID = Prompt.ask("\n> Kopyalamak istediğiniz Sunucunun ID sini girin")
    sleep(0.5)
